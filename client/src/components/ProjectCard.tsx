...
export default function ProjectCard({ title, description, image, github, live, huggingface, techStack }: ProjectCardProps) {
      ...
      >
        ...
        <div className="relative h-44 overflow-hidden">
          ...
          <div className="absolute bottom-3 left-4 right-4">
            <h3 className="text-base font-semibold text-white drop-shadow-lg">{title}</h3>
          </div>
        </div>
        ...
        <div className="p-4 pt-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {techStack.map((skill, i) => (
              <span
                key={i}
                className="px-2 py-0.5 text-[11px] font-medium rounded-full bg-cyan-400/10 text-cyan-300/80 border border-cyan-400/10"
              >
                {skill}
              </span>
            ))}
